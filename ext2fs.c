...
int ext2_block_used(uint32_t blkaddr) {
  ...
  int used = 0;
  /* TODO */
  used = group_desc[(blkaddr - 1) / blocks_per_group].gd_b_bitmap & (1 << ((blkaddr - 1) % blocks_per_group)) ? 1 : 0;
  return used;
}
...
int ext2_inode_used(uint32_t ino) {
  ...
  int used = 0;
  /* TODO */
  used = group_desc[(ino - 1) / inodes_per_group].gd_i_bitmap & (1 << ((ino - 1) % inodes_per_group)) ? 1 : 0;
  return used;
}
...
static int ext2_inode_read(off_t ino, ext2_inode_t *inode) {
  ...
  if(ext2_inode_used(ino) == 1) return 0;
  (void)ino;
  (void)inode;
  return ENOENT;
}
...
static uint32_t ext2_blkptr_read(uint32_t blkaddr, uint32_t blkidx) {
  /* TODO */
  if(blkidx < 13) return ((uint32_t *)blkaddr)[blkidx - 1];
  if(blkidx < 268) return ((uint32_t **)blkaddr)[12][blkidx - 13];
  if(blkidx < 65804) return ((uint32_t ***)blkaddr)[13][(blkidx - 268) / 256][(blkidx - 268) % 256];
  return ((uint32_t ****)blkaddr)[14][(blkidx - 65804) / (256 * 256)][((blkidx - 65804) / 256) % 256][(blkidx - 65804) % (256 * 256)];
...
long ext2_blkaddr_read(uint32_t ino, uint32_t blkidx) {
  ...
  if(blkidx >= inode.i_nblock / (BLKSIZE / 512))
    return ext2_blkptr_read(inode.i_blocks, blkidx);
...
int ext2_read(uint32_t ino, void *data, size_t pos, size_t len) {
  ...
  if((uint64_t)inode.i_size + (((uint64_t)inode.i_size_high) << 32) <= pos + len){
    return EINVAL;
  }
...
int ext2_readdir(uint32_t ino, uint32_t *off_p, ext2_dirent_t *de) {
  ...
  if(!ext2_read(ino, de, *off_p, EXT2_DIRSIZE(0))){
    if(!ext2_read(ino, de, *off_p, EXT2_DIRSIZE(0) + de->de_namelen + 1)) return 1;
  }
  (void)ino;
  (void)off_p;
  (void)de;
...
int ext2_readlink(uint32_t ino, char *buf, size_t buflen) {
  ...
  if((inode.i_mode & 61440) == EXT2_IFLNK){
    if(buflen < 60){
      strncpy(buf, inode.i_blocks, buflen);
      return 0;
    } else{
      if(!ext2_read(ino, buf, 0, buflen)) return 0;
    }
  }
  return EINVAL;
  (void)buf;
...
int ext2_lookup(uint32_t ino, const char *name, uint32_t *ino_p,
  ...
  /* TODO */
  if(inode.i_mode & 61440 != EXT2_IFDIR) return ENOTDIR;
  uint32_t off_p = 0;
  ext2_dirent_t de;
...
int ext2_mount(const char *fspath) {
  ...
  group_desc = blkdata[EXT2_GDOFF / BLKSIZE];
